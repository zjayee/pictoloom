...
export const nextRoundTrigger: AppUpgradeDefinition = {
  ...
  onEvent: async (_, context) => {
    try {
      // Clear existing jobs
      const prevDrawJobId = await context.redis.get(
        "scheduledNextDrawRoundJobId"
      );
      if (prevDrawJobId) {
        await context.scheduler.cancelJob(prevDrawJobId);
        console.log("Cancelled existing post job with id:", prevDrawJobId);
      }
      const prevGuessJobId = await context.redis.get(
        "scheduledNextGuessRoundJobId"
      );
      if (prevGuessJobId) {
        await context.scheduler.cancelJob(prevGuessJobId);
        console.log("Cancelled existing post job with id:", prevGuessJobId);
      }
      ...
      await context.redis.set("scheduledNextRoundJobId", drawJobId);
      await context.redis.set("scheduledNextRoundJobId", guessJobId);
